...
bool parse_bool(const std::string & value, const std::string & option)
{
  ...
  throw std::runtime_error(option + " must be true or false, got: " + value);
}

struct FeedbackEvent {
  std::string speech;
  std::optional<std::array<uint8_t, 3>> led_rgb;
};

std::optional<std::array<uint8_t, 3>> mode_led_color(const std::string & mode)
{
  if (mode == "loco") return std::array<uint8_t, 3>{0, 255, 0};
  if (mode == "vr") return std::array<uint8_t, 3>{0, 0, 255};
  if (mode == "npz") return std::array<uint8_t, 3>{128, 0, 255};
  return std::nullopt;
}

class RobotFeedback {
 public:
  explicit RobotFeedback(bool voice_enabled) : voice_enabled_(voice_enabled)
  {
    client_.Init();
    client_.SetTimeout(2.0f);
    worker_ = std::thread([this]() { run(); });
    std::cout << "[G1Bridge] voice announcements " << (voice_enabled_ ? "enabled" : "disabled")
              << "; mode LED control enabled" << std::endl;
  }

  ~RobotFeedback() { close(); }

  void enqueue(FeedbackEvent event)
  {
    if (!voice_enabled_) event.speech.clear();
    if (event.speech.size() > 128) {
      std::cerr << "[G1Bridge] Ignore oversized voice announcement" << std::endl;
      event.speech.clear();
    }
    if (event.speech.empty() && !event.led_rgb.has_value()) return;
    {
      std::lock_guard<std::mutex> lock(mutex_);
      constexpr size_t kMaxQueuedFeedback = 8;
      if (queue_.size() >= kMaxQueuedFeedback) {
        queue_.pop_front();
        std::cerr << "[G1Bridge] Feedback queue full; dropped oldest event" << std::endl;
      }
  ...
  void close()
  {
    ...
    try {
      const int32_t result = client_.LedControl(0, 0, 0);
      if (result != 0) {
        std::cerr << "[G1Bridge] LED shutdown failed ret=" << result << std::endl;
      }
    } catch (const std::exception & error) {
      std::cerr << "[G1Bridge] LED shutdown exception: " << error.what() << std::endl;
    }
  }

 private:
  void apply_led(const std::array<uint8_t, 3> & rgb, bool log_success)
  {
    try {
      const int32_t result = client_.LedControl(rgb[0], rgb[1], rgb[2]);
      if (result == 0) {
        if (log_success) {
          std::cout << "[G1Bridge] LED RGB=(" << static_cast<int>(rgb[0]) << ","
                    << static_cast<int>(rgb[1]) << "," << static_cast<int>(rgb[2]) << ")"
                    << std::endl;
        }
      } else {
        std::cerr << "[G1Bridge] LED control failed ret=" << result << std::endl;
      }
    } catch (const std::exception & error) {
      std::cerr << "[G1Bridge] LED control exception: " << error.what() << std::endl;
    }
  }

  void run()
  {
    constexpr auto kLedRefreshInterval = std::chrono::seconds(1);
    std::optional<std::array<uint8_t, 3>> active_led_rgb;
    while (true) {
      FeedbackEvent event;
      bool have_event = false;
      {
        ...
        condition_.wait_for(
            lock, kLedRefreshInterval, [this]() { return stop_ || !queue_.empty(); });
        if (stop_) return;
        if (!queue_.empty()) {
          event = std::move(queue_.front());
          queue_.pop_front();
          have_event = true;
          if (event.led_rgb.has_value()) active_led_rgb = event.led_rgb;
        }
      }

      if (have_event && !event.speech.empty()) {
        try {
          const int32_t result = client_.TtsMaker(event.speech, 1);
          if (result == 0) {
            std::cout << "[G1Bridge] spoke: " << event.speech << std::endl;
          } else {
            std::cerr << "[G1Bridge] TTS failed ret=" << result << ": " << event.speech << std::endl;
          }
        } catch (const std::exception & error) {
          std::cerr << "[G1Bridge] TTS exception: " << error.what() << std::endl;
        }
      }

      // TTS temporarily takes ownership of the light strip and restores its
      // default yellow state. Reapply the active mode color after every
      // announcement, and refresh it periodically in case another onboard
      // service overwrites the LED while the mode remains unchanged.
      if (active_led_rgb.has_value()) {
        apply_led(*active_led_rgb, have_event && event.led_rgb.has_value());
      }
    }
  }
...
RemoteState parse_remote(const std::array<uint8_t, 40> & remote)
{
  ...
  out.stop = (keys & (1U << 10)) != 0;
  out.a = (keys & (1U << 8)) != 0;
  out.b = (keys & (1U << 9)) != 0;
  out.up = (keys & (1U << 12)) != 0;
  out.right = (keys & (1U << 13)) != 0;
  out.down = (keys & (1U << 14)) != 0;
  out.left = (keys & (1U << 15)) != 0;
 ...
 public:
      ...
      const std::vector<float> & motor_temp)
  {
    ...
    meta << "{\"q\":" << ndarray_meta(q_ref) << ",\"dq\":" << ndarray_meta(dq_ref)
         ...
         << ",\"up\":" << bool_text(remote.up)
         << ",\"down\":" << bool_text(remote.down)
         << ",\"left\":" << bool_text(remote.left)
         << ",\"right\":" << bool_text(remote.right) << "},\"sticks\":{";
    meta << "\"lx\":";
...
class G1UdpBridge {
 public:
  G1UdpBridge(BridgeConfig cfg, std::string network_interface, bool voice_announcements)
      : cfg_(std::move(cfg)),
        ...
        state_sender_(cfg_.udp.state_host, cfg_.udp.state_port, cfg_.udp.sndbuf_bytes)
  {
    ...
    robot_feedback_ = std::make_unique<RobotFeedback>(voice_announcements);
    ...
    std::cout << "[G1Bridge] Waiting for a valid policy command before releasing the motion service" << std::endl;
  }
 ...
 private:
  void wait_for_initial_command()
  {
    ...
    while (!have_initial_valid_command_.load(std::memory_order_acquire)) {
      if (g_stop_requested.load(std::memory_order_relaxed)) {
        throw std::runtime_error("Interrupted while waiting for the initial policy command");
      }
      initial_command_cv_.wait_for(lock, std::chrono::milliseconds(100));
    }
    std::cout << "[G1Bridge] Valid policy command stream detected; starting low-level handoff" << std::endl;
  }
  ...
  void stdin_button_loop()
  {
    ...
    std::cout << "[G1Bridge] stdin buttons enabled: s=start, a=A, b=B, [=left, ]=right, "
                 "x=stop, q=emergency exit"
  ...
  void handle_stdin_button(char input)
  {
    ...
    if (ch == 's') {
      ...
      pulse_stdin_button(stdin_left_until_ns_, "left");
    } else if (ch == ']') {
      pulse_stdin_button(stdin_right_until_ns_, "right");
    } else if (ch == 'x') {
  ...
  RemoteState apply_stdin_button_overrides(RemoteState remote) const
  {
    ...
    remote.left = remote.left || stdin_button_active(stdin_left_until_ns_, now);
    remote.right = remote.right || stdin_button_active(stdin_right_until_ns_, now);
  ...
  void on_udp_command(const LatestPacket & packet)
  {
    ...
    try {
      ...
      if (have_previous_seq && packet_seq <= previous_seq) {
        ...
        last_announcement_id_ = 0;
        last_mode_indicator_id_ = 0;
      ...
      if (timeout_recovered) {
        std::cout << "[G1Bridge] Fresh policy command stream active after timeout" << std::endl;
      }
      dispatch_robot_feedback(packet.data);
  ...
  void dispatch_robot_feedback(const YAML::Node & command)
  {
    if (!robot_feedback_) return;
    FeedbackEvent event;
    const uint64_t announcement_id = command["announcement_id"].as<uint64_t>(0);
    if (announcement_id > last_announcement_id_) {
      event.speech = command["announcement"].as<std::string>("");
      last_announcement_id_ = announcement_id;
    }
    const uint64_t mode_indicator_id = command["mode_indicator_id"].as<uint64_t>(0);
    if (mode_indicator_id > last_mode_indicator_id_) {
      const std::string mode = command["mode_indicator"].as<std::string>("");
      event.led_rgb = mode_led_color(mode);
      last_mode_indicator_id_ = mode_indicator_id;
      if (!event.led_rgb.has_value()) {
        std::cerr << "[G1Bridge] Ignore unknown mode indicator: " << mode << std::endl;
      }
    }
    robot_feedback_->enqueue(std::move(event));
  }
  ...
  void command_watchdog_loop()
  {
    ...
    while (!command_watchdog_stop_.load(std::memory_order_relaxed)) {
      ...
      if (low_level_active_.load(std::memory_order_acquire) && command_timed_out(now) &&
        ...
        std::cerr << "[G1Bridge] Policy command timeout; waiting for recovery without changing LowCmd" << std::endl;
      }
      next += period;
  ...
  std::unique_ptr<RobotFeedback> robot_feedback_;
  uint64_t last_announcement_id_ = 0;
  uint64_t last_mode_indicator_id_ = 0;
  ...
  std::atomic<uint64_t> stdin_left_until_ns_{0};
  std::atomic<uint64_t> stdin_right_until_ns_{0};
...
ProgramOptions parse_options(int argc, char ** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--voice-announcements" && i + 1 < argc) {
      options.voice_announcements = parse_bool(argv[++i], arg);
      continue;
    }
    if (arg.rfind("--voice-announcements=", 0) == 0) {
      options.voice_announcements = parse_bool(
          arg.substr(std::string("--voice-announcements=").size()), "--voice-announcements");
      continue;
    }
    if (arg == "-h" || arg == "--help") {
      std::cout << "Usage: g1_udp_bridge [--net IFACE] [--config PATH] "
                   "[--voice-announcements true|false]" << std::endl;
      std::exit(0);
    }
...
int main(int argc, char ** argv)
{
  ...
  try {
    ...
    g1_bridge::G1UdpBridge bridge(
        std::move(config), options.network_interface, options.voice_announcements);
