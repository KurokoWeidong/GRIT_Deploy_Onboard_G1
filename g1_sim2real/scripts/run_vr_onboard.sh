...
MOTION_PATH="${GRIT_MOTION_PATH:-}"
NETWORK_INTERFACE="${G1_NET:-}"
VR_SERVER="${GRIT_VR_SERVER:-}"
VR_TIMEOUT_MS="${GRIT_VR_TIMEOUT_MS:-300}"
...
usage() {
  cat <<'EOF'
Usage: run_vr_onboard.sh --net IFACE --vr-server HOST [options]

The XRoboToolkit PC Service and teleop/serve_xrobot_teleop.py run on HOST.
The G1 runs the native bridge and C++ ONNX inference locally.

Options:
  --net IFACE              G1 internal control interface (normally eth0)
  --vr-server HOST         Wi-Fi address of the retarget workstation
  --motion-file PATH       Enable Loco/VR/NPZ cycling with one NPZ or a directory
  --vr-timeout-ms N        Return to WBC stand if poses are stale (default: 300)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --vr-server) VR_SERVER="${2:?missing value for $1}"; shift 2 ;;
    --motion-file|--motion-path) MOTION_PATH="${2:?missing value for $1}"; shift 2 ;;
    --vr-timeout-ms) VR_TIMEOUT_MS="${2:?missing value for $1}"; shift 2 ;;
...
if [[ -z "${NETWORK_INTERFACE}" || -z "${VR_SERVER}" ]]; then
  usage >&2
  exit 2
fi
...
required_paths=(
  "${BRIDGE_BIN}" "${POLICY_BIN}" "${BRIDGE_RUNNER}" "${BRIDGE_CONFIG}"
  "${CONTROLLER_CONFIG}" "${TRACKING_CONFIG}" "${POLICY_PATH}" "${LOCOMOTION_POLICY_PATH}"
)
if [[ -n "${MOTION_PATH}" ]]; then
  required_paths+=("${MOTION_PATH}")
fi
...
if ! [[ "${VR_TIMEOUT_MS}" =~ ^[1-9][0-9]*$ ]] || (( VR_TIMEOUT_MS < 100 )); then
  echo "--vr-timeout-ms must be an integer of at least 100" >&2
  exit 2
fi
...
echo "[run_vr_onboard] net=${NETWORK_INTERFACE}"
echo "[run_vr_onboard] vr_server=${VR_SERVER} ports=28701,28702,28703"
echo "[run_vr_onboard] vr_timeout_ms=${VR_TIMEOUT_MS}"
echo "[run_vr_onboard] motion=${MOTION_PATH:-disabled}"
echo "[run_vr_onboard] policy=${POLICY_PATH}"
echo "[run_vr_onboard] locomotion_policy=${LOCOMOTION_POLICY_PATH}"
echo "[run_vr_onboard] voice_announcements=${VOICE_ANNOUNCEMENTS}"

bridge_command=(bash "${BRIDGE_RUNNER}" --voice-announcements "${VOICE_ANNOUNCEMENTS}")
motion_source="vr"
if [[ -n "${MOTION_PATH}" ]]; then
  motion_source="vr_npz"
fi
policy_command=(
  "${POLICY_BIN}"
